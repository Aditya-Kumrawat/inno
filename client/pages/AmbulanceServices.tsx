...
const AMBULANCE_START: [number, number] = [22.71, 75.85];
...
export default function AmbulanceServices() {
  ...
  const [ambulances, setAmbulances] = useState<
    { id: string; pos: [number, number]; base: [number, number] }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const step = () => {
      setRouteIndex((idx) => {
        ...
        setAmbulances((list) =>
          list.map((a) =>
            a.id === selectedAmbId ? { ...a, pos: nextPos } : a,
          ),
        );
  ...
  const startAmbulance = useCallback(
    (amb: { id: string; pos: [number, number]; base: [number, number] }) => {
      if (!userPos) {
        alert(
          "Fetching your location. Please allow location access and try again.",
        );
        return;
      }
  ...
  const onBookNearest = useCallback(() => {
    ...
    const nearest = ambulances.reduce(
      (best, a) =>
        distance(a.pos, userPos) < distance(best.pos, userPos) ? a : best,
      ambulances[0],
    );
  ...
  const onCancel = useCallback(() => {
    ...
    if (selectedAmbId) {
      setAmbulances((list) =>
        list.map((a) => (a.id === selectedAmbId ? { ...a, pos: a.base } : a)),
      );
    }
  ...
  const nearestHospitals = useMemo(() => {
    ...
    return offsets.map((o, i) => ({
      id: `h${i}`,
      pos: [base[0] + o[0], base[1] + o[1]] as [number, number],
    }));
  }, [userPos]);

  const hospitals = useMemo(
    () => [HOSPITAL_POS, ...nearestHospitals.map((h) => h.pos)],
    [nearestHospitals],
  );
  useEffect(() => {
    if (ambulances.length === 0 && hospitals.length) {
      const list = hospitals.slice(0, 7).map((hp, i) => ({
        ...
        base: hp,
        pos: [
          hp[0] + (i % 2 ? 0.001 : -0.001),
          hp[1] + (i % 3 ? 0.0015 : -0.0015),
        ] as [number, number],
      }));
      setAmbulances(list);
    }
                        ...
                        <Popup>
                          <div className="space-y-2 text-sm">
                            <div className="font-medium">
                              Ambulance {amb.id.toUpperCase()}
                            </div>
                            <div>
                              Location: {amb.pos[0].toFixed(4)},{" "}
                              {amb.pos[1].toFixed(4)}
                            </div>
                            <div>
                              Destination: {amb.base[0].toFixed(4)},{" "}
                              {amb.base[1].toFixed(4)}
                            </div>
                            <Button
                              size="sm"
                              className="mt-2 w-full"
                              onClick={() => startAmbulance(amb)}
                            >
                              Book this ambulance
                            </Button>
                          </div>
              ...
              <Card className={frostedCardClass}>
                ...
                <CardContent className="grid grid-cols-3 gap-3">
                  <Button
                    ...
                    onClick={() => alert("SMS sent to Ambulance")}
                  >
                  ...
                  <Button
                    className="flex flex-col items-center gap-1 py-6"
                    onClick={() => alert("SMS sent to Police")}
                  >
                  ...
                  <Button
                    className="flex flex-col items-center gap-1 py-6"
                    onClick={() => alert("SMS sent to Fire")}
                  >
                  ...
                  {contacts.map((c, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          size="sm"
                          onClick={() => alert(`SMS sent to ${c.name}`)}
                        >
                          Notify
                        </Button>
                        <Button
              ...
              <Card className={frostedCardClass}>
                <CardContent className="p-4">
                  <Button
                    ...
                    onClick={() =>
                      alert("Calling ambulance, police, fire, and contacts")
                    }
                  >
                    Call All
                  </Button>
